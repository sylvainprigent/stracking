...
# main stracking __init__ can detect if it is being loaded by the setup
# routine, to avoid attempting to load components that aren't built yet:
# the numpy distutils extensions that are used by stracking to
# recursively build the compiled extensions in sub-packages is based on the
# Python import machinery.
builtins.__stracking_SETUP__ = True
...
import stracking
import stracking._build_utils.min_dependencies as min_deps  # noqa


VERSION = stracking.__version__
...
class CleanCommand(Clean):
    ...
    def run(self):
        ...
        if os.path.exists('build'):
            shutil.rmtree('build')
        for dirpath, dirnames, filenames in os.walk('stracking'):
...
try:
    ...
    class build_ext_subclass(build_ext):
        def build_extensions(self):
            from stracking._build_utils.openmp_helpers import get_openmp_flag

            if stracking._OPENMP_SUPPORTED:
...
# To automate release of binary packages for stracking we need a tool
...
def configuration(parent_package='', top_path=None):
    ...
    from stracking._build_utils import _check_cython_version
    ...
    config.add_subpackage('stracking')
...
def check_package_status(package, min_version):
    ...
    req_str = "stracking requires {} >= {}.\n".format(
        package, min_version)

    instructions = ("Installation instructions are available on the "
                    "stracking website: "
                    "http://github.com/sylvainprigent/stracking\n")
...
def setup_package():
    ...
    if len(sys.argv) == 1 or (
        ...
        # pip is used to install stracking when Numpy is not yet present in
        # the system.
        ...
        if sys.version_info < (3, 6):
            raise RuntimeError(
                "stracking requires Python 3.7 or later. The current"
